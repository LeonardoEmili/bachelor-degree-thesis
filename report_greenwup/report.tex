...
di questa fase ciascun nodo sarà fornito di un indirizzo di wake up $w=w_{h}w_{e}$ della lunghezza di 8 bit, con $w_{h}$ definito dalla
...
necessario a fornire veridicità alle considerazioni affermate, vedere più avanti la formula utilizzata (Equazione 1.1).\\
...
I risultati ottenuti fanno riferimento ad un numero $n$ di simulazioni eseguite attraverso il simulatore Castalia che viene calcolato utilizzando la
seguente formula:
...
con $\bar{x}$ che denota la media aritmetica dei valori osservati e $s$ la deviazione standard. Scegliamo di impiegare un intervallo di 
confidenza del 95\% e una precisione del 5\%, da cui seguono i valori $z=1.96$ e $r=5$. Inoltre, per fornire supporto ai risultati ottenuti
si considera $n=max(n_1, n_2, \ldots, n_k)$, con $n_1, n_2, \ldots, n_k$ i valori ottenuti dalla formula al variare del parametro \emph{iaTime}.
