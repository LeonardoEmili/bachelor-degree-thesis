...
superflui derivati da attese che precedono la ricezione dei pacchetti. L'idea prevede che i nodi della rete siano prevalentemente dormienti durante il loro
...
ridotte e consumi energetici nettamente inferiori rispetto a quelli della radio principale.\\

Dal punto vista energetico il meccanismo delle wake up radio è altamente efficiente e non richiede alcuna forma di sincronizzazione che è generalmente
non desiderata in quanto implicitamente introduce overhead dovuto alla gestione di timer aggiuntivi.\\
...
L'idea in questo caso consiste nel limitare la ricezione di una sequenza di wake up ad un sottoinsieme di nodi della rete, evitando quindi che questa sia
recepita da tutti i nodi presenti all'interno del range di comunicazione. Il principio fondante è quello di scegliere una o più opportune sequenze di
wake up e di assegnarle ad un certo sottoinsieme di nodi della rete. A meno di errori di interpretazione della sequenza di wake up, al momento della
...
virtualmente azzerato il numero dei nodi che vengono riattivati a seguito di sequenze di wake up non a loro destinate ma che si trovano
all'interno del loro range di comunicazione.\\
...
\cite{ctp-wur-paper} viene proposto il protocollo CTP-WUR come estensione al classico protocollo CTP che utilizza wake up radio piuttosto che il duty cycling
...
da \emph{iaTime} varia il traffico di rete in quanto la frequenza con cui i pacchetti sono generati nella rete è definita come: $1/iaTime$.
...
Analizzando la \textbf{Figura \ref{performance-energy}} si può notare come l'energia richiesta dalla rete risulta essere inferiore nel caso della variante
...
maniera significativa dal parametro \emph{energyClassRetries}. Nello scenario considerato abbiamo osservato che esse sono influenzate essenzialmente dai
