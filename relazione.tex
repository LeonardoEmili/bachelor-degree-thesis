...
Nel seguito verranno affrontate le principali problematiche che spingono la ricerca a sviluppare nuove idee per la costruzione di protocolli di rete
altamente efficienti.\\

Nel \textbf{Capitolo 2} verrà presentato lo scenario di riferimento e le principali considerazioni che nel tempo sono state adottate
e che ad oggi definiscono il panorama delle Wireless Sensor Networks.\\

Nel \textbf{Capitolo 3} verrà presentato uno specifico protocollo di rete per sistemi IoT: il protocollo GREEN-WUP \cite{novel-wake-up-receiver-paper}.
...
protocollo in questione. Analizzeremo in dettaglio il comportamento del protocollo e le sue caratteristiche. Verranno inoltre presentati i suoi principali
vantaggi ed il suo comportamento di fronte a problematiche comuni ai protocolli di comunicazione.\\
...
a cui si fa riferimento in questo documento sono stati testati mediante una versione particolare del framework \emph{GreenCastalia} \cite{greencastalia-paper}
che supporta \emph{wake up radios} e \emph{energy harvesting}.\\

Nel \textbf{Capitolo 5} saranno presentati i parametri dei protocolli di rete che in misura principale contribuiscono alle loro prestazioni e ne favoriscono
...
Nel \textbf{Capitolo 6} verranno tratte le conclusioni di quanto osservato e possibili direzioni future nell'ambito di protocolli per sistemi IoT.
...
In ambito di ricerca è possibile trovare molti paper scientifici in cui viene analizzata la transizione dall'utilizzo del duty cycling alle wake up radio.
...
In generale, l'approccio utilizzato da GREEN-WUP non garantisce
...
numero minore rispetto alla versione originale.\\

In particolare, la massima efficienza energetica è raggiunta in condizioni di elevato stress
...
Si noti come in \textbf{(b)} il \emph{Packet Delivery Ratio} ottenuto della variante sia leggermente inferiore rispetto a quello della versione originale.
Questo segue dal fatto che, come discusso in \textbf{Capitolo 4.1}, esiste una probabilità che i nodi non si sveglino a seguito della ricezione di un
messaggio di wake up e che il pacchetto venga scartato in seguito al raggiungimento del numero massimo di ritrasmissioni. Si faccia riferimento al
\textbf{Capitolo 5} dove sono esposte particolari osservazioni per migliorare diversi aspetti delle prestazioni ottenute della variante proposta.\\
...
derivati dall'introduzione di questa fase siano inferiori in ordini di grandezza rispetto ai costi energetici dovuti al normale funzionamento della rete. Inoltre, per
ragioni di efficienza durante l'implementazione di questa nuova fase si è scelto di distinguere i pacchetti \emph{DATA}, utilizzati durante la fase di scambio
dati, dai pacchetti \emph{INTEREST}. Questo dettaglio implementativo garantisce che i pacchetti che realizzano la fase di interest dissemination
possono essere scambiati solo all'inizio e se ricevuti successivamente saranno scartati.
...
\textbf{(a)}. In effetti, in tutte le sperimentazioni attuate l'overhead registrato è costantemente al di sotto del joule per ciascun
valore assunto dal parametro $\emph{iaTime}=\{2,5,10,25,50\}$. In questo caso mostriamo solamente le variazioni energetiche causate dall'introduzione
di questa fase in quanto gli hop count determinati coincidono con quelli stimati nella versione che non esegue questa fase.
Conseguentemente anche le latenze sperimentate e le prestazioni della rete in generale, ad eccezione dell'energia spesa, rimangono inalterate.\\
...
I risultati relativi alla fase di interest dissemination sopra esposti fanno riferimento all'implementazione dell'algoritmo descritto in \textbf{Capitolo 3.2}.
...
\begin{thebibliography}{9}
    ...
    \bibitem{novel-wake-up-receiver-papeaaar}
    C. Petrioli, D. Spenza, P. Tommasino, A. Trifiletti.
    \say{A Novel Wake-Up Receiver with Addressing Capability for Wireless Sensor Nodes}.
    In: \textit{2014 IEEE International Conference on Distributed Computing in Sensor Systems. Marina Del Rey, CA, USA, May 26-28 2014.}
